...
export default function RevealGate({ playerName, subtitulo, revelado, onRevelar, children, onExit }) {
  return (
    <div className="flex min-h-[100dvh] flex-col items-center justify-center px-6 py-10">
      ...
      <AnimatePresence mode="wait">
        {!revelado ? (
          ...
          >
            <p className="text-xs font-medium uppercase tracking-widest text-muted">Passe o celular para</p>
            <div className="mt-6 flex flex-col items-center gap-5 rounded-2xl border border-border bg-surface px-8 py-14 shadow-[0_20px_60px_-20px_rgba(0,0,0,0.6)]">
              ...
              <div>
                ...
                {subtitulo && <p className="mt-1 text-sm text-secondary">{subtitulo}</p>}
              </div>
              <motion.button
                whileTap={{ scale: 0.96 }}
                onClick={onRevelar}
